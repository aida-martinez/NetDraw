...
NetDraw is a spreadsheet-style plugin designed specifically for knockout tennis tournaments — think of it as "TablePress, but built for brackets instead of standard tables."
...
- **Draw Types** — Choose between a **Standard** single-elimination draw and a **Qualifying** draw that stops as soon as the configured number of players secure advancement spots.
...
- **No Bloat** — Built purely with Vanilla JS and native CSS — no jQuery or external visual dependencies.
...
1. Download the `netdraw.zip` file from the plugin page or the repository.
...
```
netdraw/
├── netdraw.php               # Main plugin bootstrap & WordPress hooks
├── uninstall.php             # Cleanup handler run on plugin deletion
├── readme.txt                # WordPress.org readme (SVN)
├── README.md                 # GitHub readme
└── assets/
    ├── css/
    │   ├── admin.css         # Custom styling for the backend meta box
    │   └── frontend.css      # Tree styling, connector lines & colours
    └── js/
...
```json
{
  ...
  "matches": {
    ...
    "r1_m3": { "p1": "Sinner", "p2": "Zverev",    "score": "",        "winner": ""   },
    "r1_m4": { "p1": "Alcaraz", "p2": "Rublev",   "score": "",        "winner": ""   }
  }
}
```
...
- **Next match index** = ceiling( m / 2 ), where `m` is the current match number within the round.
- If `m` is **odd**, the winner becomes `p1` (Player 1) of the next match.
- If `m` is **even**, the winner becomes `p2` (Player 2) of the next match.
- Subsequent rounds are set to **read-only** in the admin panel to prevent data out-of-sync issues, ensuring the visual progression matches actual results.
...
NetDraw is licensed under the [GPL-2.0-or-later](https://www.gnu.org/licenses/gpl-2.0.html).
